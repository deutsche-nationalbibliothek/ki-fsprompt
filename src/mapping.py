...
class Mapping:
    ...
    def query_vector_database(self, candidate: str):
        ...
        query = unicodedata.normalize("NFC", query)  # normalize unicode
        max_retries = 10
                    ...
                    result = {
                        candidate: {
                            ...
                            "cosine_similarity": np.dot(
                                embedding, response.vector["label_vector"]
                            ),
                        }
                    }
                else:
...
class LLMMapping:
    ...
    def __init__(
        ...
        Args:
            ...
            params_file (str): path to the shared parameters YAML file
        """

        with open(params_file, "r") as file:
            self.shared_params = yaml.safe_load(file)
        self.p_mapping = self.shared_params["mapping"]

        self.debug = False
        self.collection_name = self.shared_params["vocab"]["weaviate_collection_name"]
...
def execute():
    ...
    parser.add_argument(
        "--params_file", help="Path to the shared parameters YAML file", type=str, required=True
    )
    ...
    log_file_path = Path("logs/LLMMapping.log")
